...
function checkIfGoblinEncountered(hero) {
    ...
    if (diceRoll <= 6) {
        showGoblin();
        showDamageIcon(heroImage.offsetLeft, heroImage.offsetTop);
        setTimeout(hideGoblin,  1500);
...
document.getElementById("takeStepButton").addEventListener("click", () => {
    ...
    if (hero.steps % 15 != 0 ) {
        ...
        hideButton("buySwordButton");
        hideButton("buyPotionButton");
        hideButton("buyShieldButton");
        hideButton("leaveShopMenuButton");
    }
});
...
function updateGameBoard(message) {
    showUpdateContainer();
    setTimeout(hideUpdateContainer,  1000);
